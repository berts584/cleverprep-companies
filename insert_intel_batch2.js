...
const newRolesContent = fs.readFileSync('temp_intel_roles_batch2.js', 'utf8');

// Find lyft section to locate end of Intel
...
// Find the last closing brace of Intel section before lyft
const beforeLyft = mainContent.substring(0, lyftStart);
const lastRoleEnd = beforeLyft.lastIndexOf("    }");

if (lastRoleEnd === -1) {
  console.error("Could not find last role end in Intel section");
  process.exit(1);
}
...
console.log("Intel batch 2 roles inserted successfully!");

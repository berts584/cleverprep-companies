...
const newRolesContent = fs.readFileSync('temp_ibm_roles_batch2.js', 'utf8');

// Find lyft section to locate end of IBM
...
// Find the last closing brace of IBM section before lyft
const beforeLyft = mainContent.substring(0, lyftStart);
const lastRoleEnd = beforeLyft.lastIndexOf("    }");

if (lastRoleEnd === -1) {
  console.error("Could not find last role end in IBM section");
  process.exit(1);
}
...
console.log("IBM batch 2 roles inserted successfully!");
